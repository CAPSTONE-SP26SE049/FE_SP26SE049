...
import { LayoutDashboard, Users, BookOpen, Award, Settings, Sparkles, ChevronLeft, ChevronRight } from 'lucide-react'
...
const NAV_ITEMS = [
    ...
    { key: '/admin/chapters', icon: BookOpen, label: 'Quản lý chương học' },
    { key: '/admin/achievements', icon: Award, label: 'Quản lý thành tựu' },
                    ...
                    <div className="flex items-center h-[72px] px-5 border-b border-purple-50 flex-shrink-0 overflow-hidden gap-3">
                        ...
                        {!collapsed && (
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                                ...
                                <div className="text-[10px] font-bold uppercase tracking-widest text-purple-400 mt-0.5">Admin Portal</div>
                            </motion.div>
                        )}
                    </div>
                        ...
                        <Dropdown menu={userMenu} placement="topRight" trigger={['click']}>
                            <div className={`flex items-center cursor-pointer p-2 rounded-2xl hover:bg-purple-50 transition-all ${collapsed ? 'justify-center' : 'gap-3'}`}>
                                <Avatar icon={<UserOutlined />} size={40}
                                    className="bg-purple-100 text-purple-600 border-2 border-purple-200 flex-shrink-0"
                                    src={(user as any)?.avatar}
                                />
                                {!collapsed && (
                                    <div className="flex-1 min-w-0">
                                        <div className="text-sm font-bold text-gray-800 truncate">{user?.fullName || 'Admin'}</div>
                                        <div className="text-[10px] text-purple-500 uppercase tracking-widest font-black">Quản trị viên</div>
                                    </div>
                                )}
                            </div>
                        </Dropdown>
                    ...
                    <Dropdown menu={userMenu} placement="bottomRight" trigger={['click']}>
                        <div className="flex items-center gap-2.5 cursor-pointer px-3 py-1.5 rounded-xl hover:bg-purple-50 transition-all">
                            <div className="hidden md:block text-right">
                                <div className="text-sm font-bold text-gray-800 leading-tight">{user?.fullName || 'Admin User'}</div>
                                <div className="text-[10px] text-purple-500 font-bold uppercase tracking-wider">Quản trị viên</div>
                            </div>
