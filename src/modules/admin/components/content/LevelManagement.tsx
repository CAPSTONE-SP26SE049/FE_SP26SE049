...
const LevelManagement: React.FC = () => {
    ...
    const getDialectDisplay = (record: any) => {
        if (record.dialect?.name) {
            return REGION_MAP[record.dialect.name.toUpperCase()] || record.dialect.name;
        }
        const dId = record.dialectId || record.dialect?.id;
        const found = dialects.find((d: any) => d.id === dId);
        return found ? (REGION_MAP[found.name?.toUpperCase()] || found.name) : '—';
    };
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (_: any, record: any) => (
                <Tag color="blue">{getDialectDisplay(record)}</Tag>
            ),
        },
        {
            title: 'Thứ Tự',
