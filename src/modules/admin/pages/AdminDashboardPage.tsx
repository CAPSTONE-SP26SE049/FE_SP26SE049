        ...
        <article className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="space-y-2.5 text-sm">
            ...
            <div className="space-y-1.5 pt-1">
              ...
              <div className="flex items-center justify-between rounded-xl bg-slate-50 px-3 py-2">
                <span className="flex items-center gap-2 text-slate-600 text-xs">
                  <TrendingUp size={14} className="text-blue-500" /> Gemini Flash (Cloud)
                </span>
                <span className={`font-bold text-[12px] ${health.geminiStatus === 'CONNECTED' ? 'text-emerald-600' : 'text-amber-500'}`}>
                  {health.geminiStatus || 'OFFLINE'}
                </span>
              </div>
            </div>
