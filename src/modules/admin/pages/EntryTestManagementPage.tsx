...
const REGION_MAP: Record<string, string> = {
  ...
  SOUTH: "Miền Nam",
  NORTH_NL: "Bắc (N/L)",
  CENTRAL_DGIR: "Trung (D/GI/R)",
  SOUTH_TRCH: "Nam (TR/CH)",
};
...
const formatRegion = (raw?: string | null): string => {
  if (!raw) return "—";
  const normalized = raw.trim().toUpperCase();
  return REGION_MAP[normalized] ?? REGION_MAP[raw] ?? raw;
};

const formatErrorType = (raw?: string | null): string => {
  if (!raw || !raw.trim()) return "Không phát hiện lỗi đặc thù";
  const normalized = raw.trim().toUpperCase();
  return ERROR_TYPE_MAP[normalized] ?? raw;
};
...
> = {
  NORTH_NL: {
    label: REGION_MAP.NORTH_NL,
    color: "text-blue-600",
    bg: "bg-blue-50",
    border: "border-blue-200",
  },
  CENTRAL_DGIR: {
    label: REGION_MAP.CENTRAL_DGIR,
    color: "text-orange-600",
    bg: "bg-orange-50",
    border: "border-orange-200",
  },
  SOUTH_TRCH: {
    label: REGION_MAP.SOUTH_TRCH,
    color: "text-emerald-600",
    bg: "bg-emerald-50",
    border: "border-emerald-200",
  },
  NORTH: {
    label: REGION_MAP.NORTH,
    color: "text-blue-600",
    bg: "bg-blue-50",
    border: "border-blue-200",
  },
  CENTRAL: {
    label: REGION_MAP.CENTRAL,
    color: "text-orange-600",
    bg: "bg-orange-50",
    border: "border-orange-200",
  },
  SOUTH: {
    label: REGION_MAP.SOUTH,
    color: "text-emerald-600",
    bg: "bg-emerald-50",
    border: "border-emerald-200",
  },
...
const EntryTestManagementPage: React.FC = () => {
  ...
  const openEdit = (q: Question) => {
    ...
    form.setFieldsValue({
      ...
      regionCategory: q.regionCategory,
    });
    setModalOpen(true);
  };
    ...
    },
    {
      ...
      render: (val) => {
        ...
        return (
          <div
            className={clsx(
              "inline-flex items-center gap-2 px-4 py-1.5 rounded-full border-[2px] font-black uppercase tracking-widest text-[10px]",
              info.bg,
              info.color,
              info.border,
            )}
    ...
    },
    {
      ...
      render: (val) => {
        ...
        return (
          <div
            className={clsx(
              "inline-flex items-center justify-center w-16 h-8 rounded-xl border-[2px] font-black text-xs",
              colorClass,
            )}
          >
            {score}%
    ...
    },
    {
      ...
      render: (val) => (
        <div className="text-right pr-4">
          ...
          <div className="text-[10px] font-bold text-slate-400 tracking-tighter opacity-70 italic">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 pb-2">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-900 uppercase tracking-tight">
              Cấu hình bài test đầu vào
            </h1>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mt-1">
              Quản lý câu hỏi chẩn đoán & thống kê kết quả đầu vào
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          >
            <RefreshCw
              size={16}
              className={clsx((loadingQ || loadingR) && "animate-spin")}
              strokeWidth={3}
            />
            Làm mới
          </motion.button>
          ...
          return (
            ...
            >
              <div className="h-full rounded-3xl border-[3px] border-slate-900 bg-white p-6 shadow-[6px_6px_0_#1f2937] transition-all hover:shadow-[10px_10px_0_#1f2937] flex flex-col justify-between overflow-hidden">
                ...
                <div
                  className={clsx(
                    "mt-6 w-12 h-12 rounded-2xl border-[2.5px] border-slate-900 shadow-[3px_3px_0_#1f2937] flex items-center justify-center transition-transform group-hover:rotate-6",
                    card.bg,
                  )}
                >
                  <Icon
      ...
      <div className="flex items-center gap-2 p-1.5 bg-slate-900/5 rounded-3xl w-fit">
        <button
          ...
          className={clsx(
            ...
            activeTab === "questions"
              ? "bg-white text-slate-900 shadow-sm border-[2.5px] border-slate-900 -translate-y-0.5"
      ...
      >
        <Form form={form} layout="vertical" className="mt-8 space-y-4">
          ...
          >
            <Input.TextArea
              rows={4}
              placeholder="Ví dụ: Nửa đêm nhai nếp nương nanh..."
        ...
        title={
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-3 mt-1">
              <span className="px-3 py-0.5 rounded-full bg-slate-900 text-white text-[9px] font-black uppercase tracking-widest">
                {selectedResult?.userEmail}
              </span>
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic">
                {selectedResult?.detectedRegion
                  ? formatRegion(selectedResult.detectedRegion)
                  : "Chưa xác định"}
              </span>
            </div>
          </div>
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-3 gap-6 mb-8">
                    <div className="p-6 bg-emerald-50 border-[3px] border-slate-900 rounded-[2rem] shadow-[4px_4px_0_#1f293705] text-center">
                      <div className="text-3xl font-black text-emerald-600 italic">
                        {Math.round(selectedResult?.overallScore || 0)}%
                      </div>
                      <div className="text-[9px] font-black uppercase tracking-widest text-emerald-700/50 mt-1">
                        ĐỘ CHÍNH XÁC
                      </div>
                    </div>
                    <div className="p-6 bg-blue-50 border-[3px] border-slate-900 rounded-[2rem] shadow-[4px_4px_0_#1f293705] text-center">
                      <div className="text-3xl font-black text-[#49B6E5] italic">
                        {details.length}
                      </div>
                      <div className="text-[9px] font-black uppercase tracking-widest text-blue-700/50 mt-1">
                        Câu trả lời
                      </div>
                    </div>
                    <div className="p-6 bg-orange-50 border-[3px] border-slate-900 rounded-[2rem] shadow-[4px_4px_0_#1f293705] text-center">
                      <div className="text-2xl font-black text-orange-500 uppercase tracking-tighter truncate">
                        {selectedResult?.detectedRegion
                          ? formatRegion(selectedResult.detectedRegion)
                          : "Chưa xác định"}
                      </div>
                      <div className="text-[9px] font-black uppercase tracking-widest text-orange-700/50 mt-1">
                        VÙNG MIỀN
                      </div>
                    </div>
                  </div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex justify-between items-start gap-4 mb-4">
                          ...
                          <div
                            className={clsx(
                              "px-5 py-2 rounded-2xl border-[2.5px] border-slate-900 font-black text-sm italic shadow-[3px_3px_0_#1f293705]",
                              isGood
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="p-4 bg-slate-50 rounded-2xl border-[2px] border-slate-900/5">
                            ...
                            <div className="font-bold text-slate-700 italic">
                              "{item.rawText || "—"}"
                            </div>
                          </div>
                          <div className="p-4 bg-slate-50 rounded-2xl border-[2px] border-slate-900/5">
                            ...
                            >
                              {formatErrorType(item.detectedError)}
                            </div>
                            {item.isRegional && (
                ...
                <div className="p-10 text-center space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs font-bold text-slate-400 italic">
                      Bản ghi cũ không tương thích với bộ giải mã JSON mới.
                    </p>
                  </div>
