...
const StudentsPage: React.FC = () => {
  ...
  const columns = [
    ...
    { title: <span className="text-xs font-bold text-gray-400 uppercase tracking-wide">Học viên</span>, key: 'student', render: (_: unknown, record: StudentAccount) => (<div className="flex items-center gap-3"><Avatar src={record.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${record.id}`} icon={<UserOutlined />} className="w-10 h-10 rounded-xl" style={{ borderRadius: 10 }} /><div><div className="font-bold text-gray-800 text-sm leading-tight">{record.fullName}</div><div className="text-xs text-gray-400">{record.email}</div></div></div>) },
