...
import { Mic, Map, Award, Globe, Brain, Sparkles, ArrowRight, User, LogOut } from '../lib/icons';
                    ...
                    <div className="hidden sm:flex items-center gap-4">
                        {isAuthenticated ? (
                            <div className="flex items-center gap-3">
                                <Button variant="ghost" onClick={handleLogin} className="hover:bg-gray-100/50 font-bold flex items-center gap-2">
                                    <User size={18} />
                                    {session?.user?.fullName || 'Tài khoản'}
                                </Button>
                                <Button variant="secondary" size="sm" onClick={logout} className="text-gray-500 hover:text-red-500 border-none bg-transparent hover:bg-red-50 rounded-full h-10 w-10 p-0 flex items-center justify-center transition-colors" title="Đăng xuất">
                                    <LogOut size={18} />
                                </Button>
                            </div>
                        ) : (
                ...
                <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
                    ...
                    <motion.div initial="hidden" animate="visible" variants={containerVariants} className="space-y-8 mt-12 md:mt-0">
                        ...
                        <motion.div variants={itemVariants} className="flex flex-row gap-3 w-full sm:w-auto">
                            {isAuthenticated ? (
                                <Button className="flex-1 sm:flex-none text-base sm:text-lg px-6 sm:px-10 py-4 shadow-xl shadow-purple-300/40 hover:shadow-purple-300/60 transition-all hover:scale-105 active:scale-95 font-extrabold whitespace-nowrap bg-gradient-to-r from-purple-600 to-indigo-600" onClick={handleStart}>Vào học ngay<ArrowRight className="ml-2" /></Button>
                            ) : (
                                <>
